...
namespace obelisk {
    ...
    public:
        static std::unique_ptr<http_response> handle(http_request &request, const sahara::string& params) {
...
//            auto token_itr = authorization_header.ifind_first("bearer ");
...
//            request.params().set("__current_user_id", result->get<rosetta::string>(0, "user_id"));
        ...
        static std::unique_ptr<http_response> permission(http_request &request, const sahara::string& params) {
